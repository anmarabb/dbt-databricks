...
left join {{ source('erp', 'financial_administrations') }} as fn on fn.id = pt.financial_administration_id
left join {{ source('erp', 'bank_accounts') }} as ba on pt.bank_account_id = ba.id
