with
  ...
  prep_stock_count as (select warehouse_id, count(*) as stock_count from {{source('erp', 'stocks')}} as st group by 1),
  prep_reseller_count as (select warehouse_id, count(*) as reseller_count from {{ source('erp', 'users') }} as u where u.customer_type = 0 group by 1)
select 
